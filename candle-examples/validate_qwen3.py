#!/usr/bin/env python3
"""
Validation script for Qwen3 candle implementation.
Compares outputs between HuggingFace transformers and candle.

Usage:
    python validate_qwen3.py --prompt "The capital of France is" --max-tokens 20
    python validate_qwen3.py --prompt "Explain quantum computing" --max-tokens 50 --chat
"""
...
def generate_reference(
    ...
    if use_chat_template:
        ...
        formatted_tokens = tokenizer.encode(formatted_prompt, add_special_tokens=False)
        ...
        # Check for specific Qwen tokens
...
def main():
    parser = argparse.ArgumentParser(description="Validate Qwen3 candle implementation")
    parser.add_argument("--model-id", default="Qwen/Qwen3-0.6B",
                        help="Model ID on HuggingFace (default: Qwen3-0.6B)")
    ...
    parser.add_argument("--thinking", action="store_true",
                        help="Enable thinking/reasoning mode in system prompt")
    ...
    parser.add_argument("--candle-output", type=str,
                        help="Path to candle output file for comparison (optional)")
    ...
    print("\n" + "=" * 80)
    print("INSTRUCTIONS FOR CANDLE/WASM")
    print("=" * 80)
    ...
    if args.chat:
        print("\n" + "=" * 80)
        print("FORMATTED PROMPT FOR RUST")
        print("=" * 80)
        print("\nFormatted prompt (repr - shows escape sequences):")
        print(repr(result['formatted_prompt']))
        print("\nFormatted prompt (readable):")
        print(result['formatted_prompt'])
        print("\nThis is what your Rust format_prompt() function should produce!")
        print("=" * 80)
    ...
    if args.temperature == 0.0:
        print("\n" + "=" * 80)
        print("NOTE: Temperature is 0.0 (greedy decoding)")
        print("=" * 80)
        print("The outputs should be EXACTLY the same (token-for-token).")
        print("If they differ, check:")
        print("  1. Chat template formatting matches exactly")
        print("  2. Tokenizer settings are identical")
        print("  3. Model weights are the same (GGUF quantization can differ)")
        print("  4. No differences in logits processing")
    else:
        print(f"\n" + "=" * 80)
        print(f"NOTE: Temperature is {args.temperature} (sampling)")
        print("=" * 80)
        print("Outputs may differ due to sampling.")
        print("Check that outputs are reasonable and similar in quality.")
