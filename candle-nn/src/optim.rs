...
/// Per-step precomputed AdamW coefficients.
struct AdamStep<'a> {
...
/// and merge that batch's gradients into an internal [`GradStore`], then
/// call [`step`](Self::step) once to average the K accumulated gradients
/// and apply a single optimizer update. This yields an effective batch
/// size of `K × micro_batch` without holding K computation graphs alive
/// simultaneously.
///
...
impl<O: Optimizer> GradAccumulator<O> {
    ...
    pub fn accumulate(&mut self, loss: &Tensor) -> Result<()> {
        ...
        match &mut self.accum {
            ...
            // changes that might introduce it.
            None => {
...
mod tests {
    ...
    /// SGD step: theta <- theta - lr * grad. Accumulating K batches then
    /// stepping with lr should match stepping once with the averaged loss.
    #[test]
