...
// ---- Packed Q8_0 (llama block_q8_0x4): 4 rows interleaved in 4-byte chunks so
// the NEON kernels broadcast one activation load across 4 output columns via
// SDOT lane selection. Built once per weight tensor (cached on the QTensor) and
// used for BOTH prefill GEMM (4x4 tiles) and decode GEMV, mirroring llama.cpp's
// aarch64 online repack for Q8_0.
...
#[cfg(all(target_arch = "aarch64", target_feature = "dotprod"))]
pub(crate) fn q8_0_packed_enabled() -> bool {
    static ON: LazyLock<bool> =
        LazyLock::new(|| std::env::var("CANDLE_Q8_0_PACKED").map_or(true, |v| v != "0"));
    *ON
}
...
mod q8_0_packed_tests {
    ...
    fn packed_q8_0_matmul_matches_baseline() {
        ...
        for m in [1usize, 3, 4, 7, 16] {
            ...
            crate::quantized::k_quants::matmul((m, k, n), &lhs, &wq, &mut want).unwrap();
