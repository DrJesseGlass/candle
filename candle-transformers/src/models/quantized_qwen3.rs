...
pub struct ModelWeights {
    // Kept quantized; only the input-token rows are dequantized per forward via
    // QTensor::embedding, instead of materializing the full f32 table at load.
...
impl ModelWeights {
    pub fn from_gguf<R: Read + Seek>(
        ...
        // Validated but unbound: the embedding is the quantized QTensor (per-row
        // dequant), not a dequantized table sized by this.
        ...
        // forward. The Arc is shared with the tied lm_head below.
        ...
        // tied, reuse the shared embed_tokens Arc: from_arc sees it shared and
        // keeps its blocks (needed for the per-row embedding) rather than releasing.
